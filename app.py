...
# Photo Reports Storage
REPORTS_FILE = "photo_reports.json"
try:
    with open(REPORTS_FILE) as f:
        PHOTO_REPORTS = json.load(f)
    print(f"Loaded {len(PHOTO_REPORTS)} photo reports")
except:
    PHOTO_REPORTS = []
    print("Starting with empty photo reports")
...
class PhotoReport(BaseModel):
    ...
    lat: float
    lng: float
    risk_score: float  # AI-generated score 0-1

class ReportReview(BaseModel):
    report_id: str
    decision: str  # "accept" or "reject"
    admin_notes: Optional[str] = ""
    admin_rating: Optional[float] = 0.5  # Risk score 0-1 set by admin
...
def get_route_segments(node_list):
    ...
    # Average walking speed: 4-5 km/h, using 4.5 km/h = 75 m/min
    WALKING_SPEED_M_PER_MIN = 75
    estimated_time = total_length / WALKING_SPEED_M_PER_MIN
    # Cap at reasonable max (2 hours for walking)
...
async def health():
    return {
        ...
        "photo_reports": len(PHOTO_REPORTS),
...
async def submit_photo_report(report: PhotoReport):
    ...
    try:
        # Decode base64 image
        if "," in report.image_data:
            image_data = report.image_data.split(",")[1]
        else:
        ...
        # If image save fails, continue without saving
        image_filename = None
    
    # Find nearest segment
    nearest_seg_id = find_nearest_segment_id(report.lat, report.lng)
    ...
    PHOTO_REPORTS.append(report_entry)
    
    # Save to file
    with open(REPORTS_FILE, "w") as f:
        json.dump(PHOTO_REPORTS, f, indent=2)
...
async def get_all_reports():
    ...
    return {
        "count": len(PHOTO_REPORTS),
        "reports": PHOTO_REPORTS
    }

@app.get("/api/reports/pending")
async def get_pending_reports():
    """Get only pending reports for admin review"""
    pending = [r for r in PHOTO_REPORTS if r["status"] == "pending"]
    return {
        "count": len(pending),
        "reports": pending
    }

@app.get("/api/reports/image/{report_id}")
async def get_report_image(report_id: str):
    """Get the image for a specific report"""
    report = next((r for r in PHOTO_REPORTS if r["id"] == report_id), None)
    if not report:
        raise HTTPException(status_code=404, detail="Report not found")
    
    if not report.get("image_file"):
        raise HTTPException(status_code=404, detail="No image available")
    
    image_path = os.path.join(UPLOADS_DIR, report["image_file"])
...
async def review_report(review: ReportReview):
    """Accept or reject a photo report"""
    report = next((r for r in PHOTO_REPORTS if r["id"] == review.report_id), None)
    if not report:
        raise HTTPException(status_code=404, detail="Report not found")
    ...
    report["status"] = "accepted" if review.decision == "accept" else "rejected"
    report["reviewed_at"] = datetime.now().isoformat()
    report["admin_notes"] = review.admin_notes
    report["admin_rating"] = review.admin_rating  # Store admin's rating
    
    score_change = 0
    
    # If accepted, update the segment score based on admin's risk rating
    if review.decision == "accept":
        seg_id = str(report["segment_id"])
        # Use admin's rating instead of original AI score
        risk_score = review.admin_rating
        
        # High risk reduces safety score, low risk increases it
        # risk_score 0.8+ = dangerous, 0.2- = safe
        if seg_id in SCORES:
            old_score = SCORES[seg_id]["score"]
            # Invert risk score: high risk -> lower safety
            safety_impact = 1 - risk_score  # 0.8 risk -> 0.2 impact
            # Blend with existing score
            new_score = round(old_score * 0.7 + safety_impact * 0.3, 2)
            new_score = max(0.1, min(1.0, new_score))
            score_change = new_score - old_score
            
            SCORES[seg_id]["score"] = new_score
            SCORES[seg_id]["confidence"] = min(1.0, SCORES[seg_id].get("confidence", 0.5) + 0.1)
    ...
    return {
        ...
        "message": f"Report {review.decision}ed successfully"
    }

def find_nearest_segment_id(lat: float, lng: float) -> int:
    ...
    for seg in SEGMENTS[:5000]:
        line = LineString(seg["coordinates"])
        dist = point.distance(line)
        if dist < min_dist:
            min_dist = dist
            nearest_id = seg["segment_id"]
